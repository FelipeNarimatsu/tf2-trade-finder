...
def main():
    ...
    #db connection
    #connection_db = mysql.connector.connect(user='root', password='root', host='trades_db', port='3306', database='db')
    #connection_db = mysql.connector.connect(user='root', password='root', host='mysql', port='3306', database='db')
    connection_db = mysql.connector.connect(user=os.getenv('MYSQL_USER'), password=os.getenv('MYSQL_PASSWORD'), host=os.getenv('MYSQL_HOST'), port='3306', database=os.getenv('MYSQL_DATABASE'))
    print("DB connected")

    # cursor = connection_db.cursor()
    # sql_command = "INSERT INTO trades (item_name, sell_price, buy_price, profit) VALUES (%s, %s, %s, %s)"
    # values = ('teste2', 11.11, 22.22 ,10) #item_name, sell_value, buy_value, profit
    ...
    #cursor = connection_db.cursor()
    #cursor.execute('CREATE TABLE trades (trade_id int not null AUTO_INCREMENT,item_name varchar(1000),sell_price float,buy_price float,profit float,PRIMARY KEY (trade_id))')
    ...
    key_price = key_price.split('–',1)[0]
    key_price = key_price.split('â€',1)[0]
    key_price = key_price.split('ŌĆō',1)[0]
    ...
    for line in item_names_list:
        ...
        if("<b>0</b> in Stock" not in full_html_stn):
            ...
            if(profit > 0):
                cursor = connection_db.cursor()
                sql_command = "INSERT INTO trades (item_name, sell_price, buy_price, profit) VALUES (%s, %s, %s, %s)"
                values = ((line.replace('\n',''), sell_price_ref, buy_price_ref ,profit)) #item_name, sell_value, buy_value, profit
                cursor.execute(sql_command,values)
                connection_db.commit()
